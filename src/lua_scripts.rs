...
-- Perform RediSearch FT.SEARCH
-- The result format is [total_results, doc1_id, [field1, value1, ...], doc2_id, [field1, value1, ...], ...]
local search_results = redis.call('FT.SEARCH', index_name, query, 'LIMIT', offset, limit, 'RETURN', 1, '$.', 'NOCONTENT')

local total_results = search_results[1]
local results = {total_results}

-- Iterate through search results, skipping the first element (total_results)
for i = 2, #search_results, 2 do
    local doc_id = search_results[i]
    -- Extract the thought key from the document ID (assuming doc_id is the Redis key)
    local thought_key = doc_id
    local thought_json = redis.call('JSON.GET', thought_key, '.')
    if thought_json then
        table.insert(results, thought_json)
    end
end
...
for _, uuid in ipairs(chain_ids) do
    ...
    local thought = redis.call('JSON.GET', thought_key, '.')
    if thought then
...
repeat
    ...
    for _, key in ipairs(result[2]) do
        local thought = redis.call('JSON.GET', key, '.')
        if thought then
