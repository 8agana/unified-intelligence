...
pub trait ThoughtRepository: Send + Sync {
    ...
    /// Get Sam's context data
    async fn get_sam_context(&self) -> Result<Option<serde_json::Value>>;
}
...
impl ThoughtRepository for RedisThoughtRepository {
    ...
    async fn get_instance_thoughts(&self, instance: &str, limit: usize) -> Result<Vec<ThoughtRecord>> {
        ...
        // Take only the keys we need to reduce batch size
        ...
        // Use batch JSON get for better performance
        ...
        let json_results: Vec<Option<serde_json::Value>> = self.redis
            .batch_json_get(&keys_and_paths)
            .await?;
        
        let mut thoughts = Vec::new();
        for json_val in json_results {
            if let Some(val) = json_val {
                // Convert to ThoughtRecord
                if let Ok(thought) = serde_json::from_value::<ThoughtRecord>(val) {
                    thoughts.push(thought);
                }
            }
        }
    ...
    async fn json_array_append(&self, key: &str, path: &str, value: &serde_json::Value) -> Result<()> {
        // Delegate to RedisManager's json_array_append
        self.redis.json_array_append(key, path, value).await
    }
    
    async fn json_set(&self, key: &str, path: &str, value: &serde_json::Value) -> Result<()> {
        // Use RedisManager's json_set method
        self.redis.json_set(key, path, value).await
    }
    
    async fn json_get_array(&self, key: &str, path: &str) -> Result<Option<Vec<serde_json::Value>>> {
        // Use RedisManager's json_get method
        self.redis.json_get_array(key, path).await
    }
    
    async fn json_delete(&self, key: &str, path: &str) -> Result<()> {
        // Use RedisManager's json_del method
        self.redis.json_del(key, path).await
    }
    
    async fn json_increment(&self, key: &str, path: &str, increment: i64) -> Result<()> {
        // Use RedisManager's json_increment method
        self.redis.json_increment(key, path, increment).await
    }
    ...
    async fn update_boost_score(&self, instance: &str, thought_id: &str, feedback_action: &str, relevance_rating: Option<i32>, dwell_time: Option<i32>) -> Result<f64> {
        ...
        // Calculate boost increment based on feedback action using config values
        let mut base_increment = match feedback_action {
            "helpful" => self.config.boost_scores.increments.helpful,
            "use" | "used" => self.config.boost_scores.increments.use_action,
            "viewed" => {
                // Award viewing boost based on dwell time
                if let Some(dwell) = dwell_time {
                    let thirty_second_periods = (dwell / 30) as f64;
                    self.config.boost_scores.increments.view_base + 
                        (thirty_second_periods * self.config.boost_scores.increments.view_per_30_seconds)
                } else { 
                    self.config.boost_scores.increments.view_base
                }
            },
            "irrelevant" => -1.0,  // Keep hardcoded for negative feedback
            _ => 0.0,
        };
    ...
    async fn apply_boost_scores(&self, instance: &str, thoughts: &mut Vec<ThoughtRecord>) -> Result<()> {
        ...
        for thought in thoughts.iter_mut() {
            ...
            if let Some(sim_score) = thought.similarity {
                // Combine text search score + boost using configured weights
                let boosted_score = sim_score * self.config.boost_scores.search_weight + 
                    (boost_score as f32 * self.config.boost_scores.boost_weight);
                thought.similarity = Some(boosted_score);
            } else {
                // For searches without scores, use boost score directly
    ...
    async fn get_identity_documents_by_field(&self, instance_id: &str, field_type: &str) -> Result<Vec<IdentityDocument>> {
        ...
        let json_results: Vec<Option<IdentityDocument>> = self.redis
            .batch_json_get(&keys_and_paths)
            .await?;
        
        let mut documents = Vec::new();
        for doc_opt in json_results {
    ...
    async fn get_all_identity_documents(&self, instance_id: &str) -> Result<Vec<IdentityDocument>> {
        ...
        // Filter out index keys before batch processing
        let document_keys: Vec<String> = keys.into_iter()
            .filter(|key| !key.ends_with(":index"))
            .collect();
        
        if document_keys.is_empty() {
            tracing::info!("🔍 No document keys found after filtering");
            return Ok(Vec::new());
        }
        
        if document_keys.len() > 0 {
            tracing::info!("🔍 First few keys: {:?}", document_keys.iter().take(3).collect::<Vec<_>>());
        }
        
        // Use batch JSON get for better performance
        let keys_and_paths: Vec<(String, String)> = document_keys.iter()
            .map(|k| (k.clone(), ".".to_string()))
            .collect();
        ...
        let mut documents = Vec::new();
        for (i, doc_opt) in json_results.into_iter().enumerate() {
            match doc_opt {
                Some(doc) => {
                    tracing::debug!("🔍 Successfully parsed document from key: {}", document_keys[i]);
                    documents.push(doc);
                },
                None => {
                    tracing::warn!("🔍 No data found for key: {}", document_keys[i]);
                }
            }
        }
    ...
    // ===== SAM METHODS IMPLEMENTATION =====
    
    async fn get_sam_context(&self) -> Result<Option<serde_json::Value>> {
        let context_key = "Sam:Context";
        
        // Try to get the JSON document
        let context_json = self.redis.json_get(context_key, "$").await?;
        
        if let Some(value) = context_json {
            // Redis returns array for $ path, extract first element
            if let serde_json::Value::Array(mut arr) = value {
                if !arr.is_empty() {
