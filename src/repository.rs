...
pub trait ThoughtRepository: Send + Sync {
    ...
    async fn search_thoughts(&self, instance: &str, query: &str, offset: i64, limit: i64) -> Result<Vec<ThoughtRecord>>;
...
impl ThoughtRepository for RedisThoughtRepository {
    ...
    async fn search_thoughts(&self, instance: &str, query: &str, offset: i64, limit: i64) -> Result<Vec<ThoughtRecord>> {
        ...
        let thought_jsons = self.redis.search_thoughts_rediseach(&index_name, query, offset, limit).await?;
