...
use crate::handlers::help::{UiHelpParams, HelpHandlerTrait};
...
impl UnifiedIntelligenceService {
    ...
    pub async fn new(redis_manager: Arc<RedisManager>, qdrant_service: QdrantService) -> Result<Self, UnifiedIntelligenceError> {
    ...
    pub async fn ui_help(
        ...
        match self.handlers.ui_help(params.0).await {
            ...
            Err(e) => {
                ...
                Err(ErrorData::internal_error(format!("Error generating help: {}", e), None))
            }
        }
    }
}
