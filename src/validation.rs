...
impl InputValidator {
    ...
    pub fn validate_thought_content(
        &self,
        content: &str,
    ...
    pub fn validate_thought_numbers(
        &self,
        number: i32,
        total: i32,
    ) -> std::result::Result<(), ValidationError> {
        ...
        if number < 1 || number > total {
            return Err(ValidationError::InvalidThoughtNumber { number, max: total });
        }
...
mod tests {
    ...
    fn test_valid_thought_content() {
        ...
        assert!(
            validator
                .validate_thought_content("Valid thought content")
                .is_ok()
        );
    }
