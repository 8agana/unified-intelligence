...
impl RedisManager {
    ...
    /// Create a new Redis manager with connection pool (legacy method)
    ...
    /// Execute multiple Redis commands in a pipeline for efficiency
    pub async fn pipeline<F, R>(&self, f: F) -> Result<R>
    where
        F: FnOnce(&mut redis::Pipeline) -> &mut redis::Pipeline + Send,
        R: redis::FromRedisValue,
    {
        let mut conn = self.get_connection().await?;
        let mut pipe = redis::pipe();
        
        // Apply the pipeline operations
        f(&mut pipe);
    ...
    /// Batch JSON get operations
    pub async fn batch_json_get<T: serde::de::DeserializeOwned>(
        &self,
        keys_and_paths: &[(String, String)],
    ) -> Result<Vec<Option<T>>> {
        if keys_and_paths.is_empty() {
            return Ok(Vec::new());
        }
        ...
        for result in results {
            match result {
                Some(json_str) => {
                    // Handle RedisJSON array response for "$" paths
                    let value = if keys_and_paths.first().map(|(_, p)| p.as_str()) == Some("$") {
                        if let Ok(values) = serde_json::from_str::<Vec<serde_json::Value>>(&json_str) {
                            if let Some(first_value) = values.first() {
                                serde_json::from_value(first_value.clone()).ok()
                            } else {
                                None
                            }
                        } else {
    ...
    pub async fn batch_set_with_ttl(&self, items: &[(String, String, i64)]) -> Result<()> {
        if items.is_empty() {
            return Ok(());
        }
        ...
        // Add all SET commands with expiration
    ...
    /// Batch increment operations (for counters)
    pub async fn batch_incr(&self, keys: &[String]) -> Result<Vec<i64>> {
        if keys.is_empty() {
            return Ok(Vec::new());
        }
    ...
    pub async fn json_array_append(&self, key: &str, path: &str, value: &serde_json::Value) -> Result<()> {
    ...
    /// Get a JSON array
    pub async fn json_get_array(&self, key: &str, path: &str) -> Result<Option<Vec<serde_json::Value>>> {
        let result: Option<serde_json::Value> = self.json_get(key, path).await?;
        
        match result {
            Some(serde_json::Value::Array(arr)) => Ok(Some(arr)),
            Some(_) => Ok(None), // Not an array
            None => Ok(None),
        }
    }
    
    /// Increment a JSON numeric field
    pub async fn json_increment(&self, key: &str, path: &str, increment: i64) -> Result<()> {
        use redis::JsonAsyncCommands;
        let mut conn = self.get_connection().await?;
    ...
    pub async fn ping(&self) -> Result<()> {
        ...
        if response == "PONG" {
            ...
            Err(UnifiedIntelligenceError::HealthCheckFailed(
