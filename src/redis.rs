...
impl RedisManager {
        ...
        match result {
            Some(bytes) => {
                let embedding: Vec<f32> = bincode::deserialize(&bytes)
                    .map_err(|e| UnifiedIntelligenceError::Internal(format!("Failed to deserialize embedding: {}", e)))?;
                Ok(Some(embedding))
            },
            None => Ok(None),
        }
    ...
    pub async fn set_cached_embedding(&self, text: &str, embedding: &[f32], ttl_seconds: i64) -> Result<()> {
        let mut conn = self.get_connection().await?;
        let key = format!("embedding:{}", hex::encode(Sha256::digest(text))); // Use SHA256 hash of text as key
        
        let bytes = bincode::serialize(embedding)
            .map_err(|e| UnifiedIntelligenceError::Internal(format!("Failed to serialize embedding: {}", e)))?;
        
        conn.set_ex::<_, _, ()>(&key, bytes, ttl_seconds as u64).await?;
        Ok(())
    }
