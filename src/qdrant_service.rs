...
use qdrant_client::qdrant::{SearchPoints, WithPayloadSelector, Value, PointId, Filter, Condition, Range, SearchParams};
...
impl QdrantService {
    pub async fn new(_instance_id: &str) -> Result<Self> {
        ...
        let client = Qdrant::from_url(&format!("http://{}:{}", qdrant_host, qdrant_port))
            .build()
            .map_err(|e| UnifiedIntelligenceError::Config(format!("Failed to create Qdrant client: {}", e)))?;
    ...
    pub async fn search_memories(
        ...
        if let Some(temp_filter) = temporal_filter {
                ...
                let (start_dt, end_dt) = match relative_timeframe.to_lowercase().as_str() {
                    "yesterday" => {
                        ...
                        (
                            ...
                            yesterday.and_hms_opt(23, 59, 59).unwrap().and_utc(),
                        )
                    }
                    ...
                    timeframe if timeframe.starts_with("past ") && timeframe.ends_with(" days") => {
                        ...
                        if parts.len() == 3 {
                            if let Ok(days) = parts[1].parse::<i64>() {
                                ...
                                warn!("Failed to parse days from relative timeframe: {}", relative_timeframe);
                                (now, now) // Fallback
                            }
                        } else {
            ...
            if !conditions.is_empty() {
                ...
                info!("Applied temporal filter with {} conditions", qdrant_filter.as_ref().unwrap().must.len());
            }
        }
        ...
        for collection_info in collections_response.collections {
            ...
            for scored_point in search_result.result {
                ...
                match self.point_to_thought(point_id, payload, scored_point.score, &collection_name) {
                    ...
                    Err(e) => {
                        warn!("Failed to convert Qdrant point to Thought from collection {}: {}", collection_name, e);
                    }
                }
            }
        }
        ...
        all_retrieved_thoughts.sort_by(|a, b| {
            b.semantic_score.unwrap_or(0.0).partial_cmp(&a.semantic_score.unwrap_or(0.0)).unwrap_or(std::cmp::Ordering::Equal)
        });
    ...
    fn point_to_thought(
        ...
        let id = match point_id.point_id_options {
            Some(PointIdOptions::Uuid(uuid)) => {
                Uuid::parse_str(&uuid)
                    .map_err(|e| UnifiedIntelligenceError::Other(anyhow::anyhow!("Invalid UUID: {}", e)))?
            }
                ...
                if let Some(thought_id_value) = json_payload.get("thought_id") {
                    if let Some(thought_id_str) = thought_id_value.as_str() {
                        Uuid::parse_str(thought_id_str)
                            .map_err(|e| UnifiedIntelligenceError::Other(anyhow::anyhow!("Invalid thought_id UUID: {}", e)))?
                    } else {
